...
/*
 * get_program_name - Extracts the program name from the source string.
 * @param source: The source string containing the program name.
 * @param destination: The destination buffer to store the extracted program name.
 */
void get_program_name(const char *source, char *destination) {
    int index;

    // Copy the source string to the destination buffer
    strlcpy(destination, source, strlen(source) + 1);

    // Find the position of the first space or null terminator in the destination buffer
    for (index = 0; destination[index] != '\0' && destination[index] != ' '; index++);

    // Null-terminate the destination buffer at the position of the first space or null terminator
    destination[index] = '\0';
}
...
tid_t process_execute(const char *file_name) {
  ...
  // Project 2-2. Argument Passing - Parse command line and get program name
...
/*
 * construct_stack - Constructs the stack for a new process.
 * @param arguments: An array of command-line arguments.
 * @param arg_count: The number of command-line arguments.
 * @param stack_pointer: The stack pointer that will be updated.
 */
void construct_stack(char **arguments, int arg_count, void **stack_pointer) {
    int total_length = 0;
    int argument_length;

    /* Push arguments in reverse order onto the stack */
    for (int i = arg_count - 1; i >= 0; i--) {
        argument_length = strlen(arguments[i]);
        *stack_pointer -= argument_length + 1;
        total_length += argument_length + 1;
        strlcpy(*stack_pointer, arguments[i], argument_length + 1);
        arguments[i] = *stack_pointer;
    }

    /* Align the stack to a multiple of 4 */
    *stack_pointer -= total_length % 4 != 0 ? 4 - (total_length % 4) : 0;

    /* Push NULL as the last word */
    *stack_pointer -= 4;
    **(uint32_t **)stack_pointer = 0;

    /* Push the addresses of arguments in reverse order */
    for (int i = arg_count - 1; i >= 0; i--) {
        *stack_pointer -= 4;
        **(uint32_t **)stack_pointer = (uint32_t)arguments[i];
    }

    /* Push the address of the arguments array */
    *stack_pointer -= 4;
    **(uint32_t **)stack_pointer = (uint32_t)(*stack_pointer + 4);

    /* Push the number of arguments (argc) */
    *stack_pointer -= 4;
    **(uint32_t **)stack_pointer = arg_count;

    /* Push the return address (set to 0 for now) */
    *stack_pointer -= 4;
    **(uint32_t **)stack_pointer = 0;
}
